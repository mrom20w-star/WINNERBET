...
export const AppleOfFortune: React.FC = () => {
  const { user, placeBet, winBet, setScreen, showToast, soundEnabled, setSoundEnabled } = useApp();
  ...
  // VIP Hacker / Predictor Bot State (كاشف التفاح السليم)
  const [hackerEnabled, setHackerEnabled] = useState<boolean>(true);
  ...
  // Hacker Auto-Pick safe apple (كاشف التفاح السليم - اختيار فوري ومضمون)
  const handleHackerAutoPick = () => {
    if (gameState !== 'playing') return;
    const activeRow = rows[currentRowIndex];
    if (!activeRow || activeRow.status !== 'active') return;

    // Find safe column not in rottenIndex
    const safeCol = [0, 1, 2, 3, 4].find((col) => !activeRow.rottenIndex.includes(col));
    if (safeCol !== undefined) {
      sound.playClick();
      handleSelectColumn(safeCol);
    }
  };
        ...
        <div className="w-full flex items-center justify-between gap-2 py-1">
          ...
          <div className="flex items-center gap-1.5">
            ...
            <button
              ...
              onClick={() => {
                sound.playClick();
                setHackerEnabled(!hackerEnabled);
              }}
              id="apple-hacker-quick-toggle"
              className={`px-2 py-1 rounded-lg text-[11px] font-bold flex items-center gap-1 border transition-all cursor-pointer ${
                hackerEnabled
                  ? 'bg-emerald-500/25 border-emerald-400/80 text-emerald-300 shadow-[0_0_10px_rgba(52,211,153,0.5)]'
                  : 'bg-black/40 border-slate-700 text-slate-400'
              }`}
              title="كاشف التفاح السليم 100%"
            >
              <Bot className="w-3.5 h-3.5 text-emerald-400" />
              <span>هاكر {hackerEnabled ? '🟢' : '⚪'}</span>
            </button>
            <button
              type="button"
                ...
                >
                  ...
                  <div className="flex-1 grid grid-cols-5 gap-1.5 sm:gap-2">
                    {[0, 1, 2, 3, 4].map((colIdx) => {
                      ...
                      const isSafe = !isRotten;
                      const isHackerTarget = hackerEnabled && isCurrentActive && isSafe;
                      ...
                      return (
                        <button
                          ...
                          className={`aspect-square rounded-full flex items-center justify-center relative transition-all duration-200 group ${
                            ...
                            isHackerTarget
                              ? 'ring-4 ring-emerald-400 shadow-[0_0_20px_rgba(52,211,153,1)] scale-105 z-20 animate-pulse'
                              : isCurrentActive
                              ? 'ring-2 ring-amber-400/80 hover:ring-amber-300 shadow-[0_0_12px_rgba(251,191,36,0.6)]'
                              : ''
                          } ${isSelected ? 'ring-2 ring-white' : ''}`}
                        >
                          {/* Hacker Prediction Target Badge on Active Row Safe Apples */}
                          {hackerEnabled && isCurrentActive && (
                            <>
                              {isSafe && !isRevealed && (
                                <div className="absolute -top-3 inset-x-0 mx-auto w-fit z-30 bg-gradient-to-r from-emerald-500 to-green-400 text-slate-950 font-black text-[8px] sm:text-[9px] px-2 py-0.5 rounded-full shadow-[0_0_12px_rgba(52,211,153,0.9)] flex items-center gap-1 animate-bounce pointer-events-none border border-white/60">
                                  <Target className="w-3 h-3 text-slate-950 shrink-0" />
                                  <span>سليمة 100% 🍏</span>
                                </div>
                              )}
                              {isRotten && !isRevealed && (
                                <div className="absolute -top-3 inset-x-0 mx-auto w-fit z-30 bg-red-600/90 text-white font-bold text-[8px] px-1.5 py-0.5 rounded-full shadow-md flex items-center gap-0.5 pointer-events-none opacity-80 border border-red-400/50">
                                  <span>فاسدة ❌</span>
                                </div>
                              )}
                            </>
                          ...
                          {!isRevealed && (
                            <div
                              className={`w-full h-full rounded-full overflow-hidden border relative flex items-center justify-center transition-all ${
                                isCurrentActive
                                  ? isSafe && hackerEnabled
                                    ? 'border-emerald-400 ring-2 ring-emerald-400 shadow-[0_0_16px_rgba(52,211,153,0.8)] bg-[#0d2a17]'
                            ...
                            >
                              ...
                              />
                              {isCurrentActive && (
                                <div className={`absolute inset-0 ${isSafe && hackerEnabled ? 'bg-emerald-400/15' : 'bg-amber-400/10'} animate-pulse pointer-events-none rounded-full`}></div>
                              )}
                              {/* Future row safe marker for hacker */}
                              {hackerEnabled && !isCurrentActive && (
                                <div className="absolute inset-0 flex items-center justify-center bg-black/40 rounded-full">
                                  {isSafe ? (
                                    <div className="w-2.5 h-2.5 rounded-full bg-emerald-400 shadow-[0_0_10px_rgba(52,211,153,1)] border border-white/80 animate-pulse"></div>
                                  ) : (
                                    <span className="text-[10px] text-red-500/80 font-bold">✕</span>
                                  )}
                                </div>
                              )}
                            </div>
        ...
        <div className="w-full flex flex-col gap-2 pt-1">
          ...
          {gameState === 'playing' && (
            <div className="w-full flex flex-col gap-2">
              ...
              {/* Hacker Auto-Pick Quick Action Button */}
              {hackerEnabled && (
                <button
                  type="button"
                  onClick={handleHackerAutoPick}
                  id="apple-hacker-auto-pick-btn"
                  className="w-full py-2.5 rounded-xl bg-gradient-to-r from-emerald-600 via-emerald-500 to-teal-600 hover:from-emerald-500 hover:to-teal-500 text-white font-black text-xs sm:text-sm shadow-lg shadow-emerald-950/60 border border-emerald-400/50 flex items-center justify-center gap-1.5 transition active:scale-98 cursor-pointer"
                >
                  <Sparkles className="w-4 h-4 text-emerald-200 animate-spin" />
                  <span>اختيار التفاحة السليمة تلقائياً بالهاكر 🎯</span>
                </button>
              )}
